      ...
      {accounts.isLoading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {accounts.data?.map((account) => (
            <AnaheimCard key={account.publicKey.toString()} account={account.publicKey} />
          ))}
        </div>
      ) : (
...
function AnaheimCard({ account }: { account: PublicKey }) {
  const { accountQuery, incrementMutation, setMutation, decrementMutation, closeMutation } = useAnaheimProgramAccount({
    account,
  })
